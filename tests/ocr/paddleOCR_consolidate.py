#!/usr/bin/env python3
"""
PaddleOCR with Geometric Consolidation
======================================

Extracts text using PaddleOCR and merges vertically-stacked label fragments
...
Usage:
    python paddleOCR_consolidate.py /path/to/deck_image.jpg
    python paddleOCR_consolidate.py /path/to/deck_image.jpg -v  # verbose
"""
...
# Disable oneDNN and force CPU mode
...
def extract_labels(image_path: str) -> tuple:
    """Extract all text labels with bounding box data using PaddleOCR."""
    img = Image.open(image_path)
    img_width, img_height = img.size
    
    # Initialize PaddleOCR (CPU mode, no Intel optimizations)
    ...
    # PaddleOCR returns list of lists (one per image, we only have one)
    if results and results[0]:
        for line in results[0]:
            ...
            # Calculate bounds from 4 corner points
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python paddleOCR_consolidate.py <image_path> [-v]")
        sys.exit(1)
    ...
    print("Step 1: Extracting text labels with PaddleOCR...")
