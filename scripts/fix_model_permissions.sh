...
echo "   export PGPASSWORD=\$AIRIS_DB_PASSWORD"
echo "   psql -h localhost -U "${AIRIS_DB_USER:-airisuser}" -d "${AIRIS_DB_NAME:-airisdb}" -c \"UPDATE insert_counter SET count_value = 0 WHERE table_name = 'chat_history';\""
echo ""
echo "2. Test the trigger:"
echo "   /iris-v3/scripts/test_trigger.sh"
echo ""
echo "3. Wait 60 seconds, then check live_memories:"
echo "   psql -h localhost -U "${AIRIS_DB_USER:-airisuser}" -d "${AIRIS_DB_NAME:-airisdb}" -c \"SELECT COUNT(*), MAX(injected_at) FROM live_memories;\""
echo ""
