...
psql -h localhost -U "${AIRIS_DB_USER:-airisuser}" -d "${AIRIS_DB_NAME:-airisdb}" -c "SELECT id, trigger_name, count_value, triggered_script, message, error, event_time FROM trigger_log ORDER BY event_time DESC LIMIT 10;"
echo ""
...
psql -h localhost -U "${AIRIS_DB_USER:-airisuser}" -d "${AIRIS_DB_NAME:-airisdb}" -c "SELECT * FROM live_memories ORDER BY injected_at DESC LIMIT 5;"
echo ""

# Check active triggers
echo "5. ACTIVE TRIGGERS ON chat_history:"
psql -h localhost -U "${AIRIS_DB_USER:-airisuser}" -d "${AIRIS_DB_NAME:-airisdb}" -c "SELECT trigger_name, event_manipulation, action_statement FROM information_schema.triggers WHERE event_object_table = 'chat_history';"
echo ""
