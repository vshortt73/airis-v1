...
from ollama.client import chat_completions
...
RERANK_WEIGHTS = {
    "sim":        0.50,   # similarity score
    "emotion":    0.10,   # valence*arousal, scaled
    "recurrence": 0.10,
    "cohesion":   0.10,
    "novelty":    0.05,
    "context":    0.20,   # category match
    "facet":      0.10,   # facet match
}
...
# Lens stage
# ==============================
async def run_lens_stage(convo: str):
    safe_convo = json.dumps(convo)
    messages = [
        {
            "role": "system",
            "content": "You are analyzing a conversation to extract its conceptual structure. Produce a JSON object with: orientation, intent, facets, shaped_queries, keywords."
        },
        {
            "role": "user",
            "content": safe_convo
    ...
    response_text = await chat_completions(
        messages=messages,
    ...
    try:
        ...
        print("❌ Failed to parse lens response:", e)
        return None

# ==============================
# Summaries
# ==============================
async def get_summaries(prompt):

    instructions = """anaylize this conversation and provide the following summariaries:
    takeaway: what is the overall idea, rule, or deigns or other piece of information you've come away from this converstion with?
    context: what is the context of the conversation?
    significance: Why is this conversation significant to you?
    tone: What is the overall tone of the conversation?
    event: describe the overall event in one or two words

    produce a json object with takeaway, context, significance, tone.
    "
...
"""
    ...
    #     "max_tokens": 512,
    #     "temperature": 0.1,
    #     "grammar_file": "/iris/grammar/summary.gbnf"
        # }
    ...
    response_text = re.sub(r'^```(?:json)?\s*|\s*```$', '', response_text.strip(), flags=re.MULTILINE)
    #raw_text = json.loads(response_text)
    #response = requests.post("http://127.0.0.1:9500/v1/completions", json=payload)
...
# ==============================
async def cognitive_recall(convo: str, embedder, conn_params, top_k, insert, show, mode, prompt):
    ...
    lens = await run_lens_stage(convo)
    ...
    summary = await get_summaries(convo)
    #summary = await get_summaries(convo)
...
if __name__ == "__main__":
    ...
    import asyncio
    injection = asyncio.run(cognitive_recall(convo, embedder, DB_CFG, args.top_k, args.insert, args.show, args.mode, args.prompt))
   #print(injection)
